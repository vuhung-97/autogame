...
class MainApp:
    ...
    def setup_ui(self):
        self.root.title("PRO MULTI-AUTO")
        ...
        # --- OPTION: CHỌN CẤP ĐỘ ---
        option_frame = tk.LabelFrame(main_container, text=" Cấp độ trang bị xóa ", font=("Segoe UI", 9, "bold"), bg="white", padx=10, pady=10)
        option_frame.pack(fill="x", pady=5)

        self.level_label = tk.Label(option_frame, text=f"Đang chọn: {LEVEL_NAMES[0]}", font=("Segoe UI", 10, "bold"), fg="#c23616", bg="white")
        self.level_label.pack()

        self.level_slider = tk.Scale(option_frame, from_=0, to=len(LEVEL_NAMES)-1, orient="horizontal", showvalue=0, command=self.on_level_change, bg="white", bd=0)
    ...
    def on_level_change(self, val):
        idx = int(val)
        self.bot.current_target_img = CHON_TRANGBI[idx]
        self.level_label.config(text=f"Đang chọn: {LEVEL_NAMES[idx]}")
